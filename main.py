...
LOG_MESSAGES = {
    ...
    "check_success": "Bot responded after {timeout} seconds",
...
class BotMonitor:
    def __init__(
        self, client: TelegramClient, target_bot: str, alert_user_ids: list[int]
    ):
        self.client = client
    ...
    async def send_alerts(self):
        alert_message = ALERT_MESSAGE.format(
            bot=self.target_bot, timeout=RESPONSE_TIMEOUT
        )
        for user_id in self.alert_user_ids:
            try:
                ...
                logger.error(
                    LOG_MESSAGES["alert_fail"].format(user_id=user_id, error=str(e))
                )
        ...
        try:
            ...
            try:
                await asyncio.wait_for(
                    self.check_event.wait(), timeout=RESPONSE_TIMEOUT
                )
                response_time = time.time() - start_time
                logger.info(
                    f"Bot responded successfully to the {COMMAND} command in {response_time:.2f} seconds"
                )
            except asyncio.TimeoutError:
                logger.warning(
                    LOG_MESSAGES["check_timeout"].format(timeout=RESPONSE_TIMEOUT)
                )
...
async def run_forever():
    while True:
        try:
            ...
            ALERT_USER_IDS = [
                int(id.strip()) for id in os.getenv("ALERT_USER_IDS", "").split(",")
            ]
